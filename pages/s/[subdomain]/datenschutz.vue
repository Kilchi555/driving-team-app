...
const { data, pending, error } = await useAsyncData(
  () => `datenschutz-${subdomain.value}-${preview.value ? 'p' : 'l'}`,
  () =>
    $fetch(`/api/public/website/${encodeURIComponent(subdomain.value)}/legal`, {
      query: { type: 'datenschutz', ...websitePreviewFetchQuery(previewParam.value) },
    }),
  { watch: [subdomain, preview] },
)

useHead(() => ({
  title: `Datenschutz | ${data.value?.tenant?.name || subdomain.value}`,
