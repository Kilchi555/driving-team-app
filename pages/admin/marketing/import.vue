    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div class="bg-white rounded-xl border p-6">
        ...
        <div class="mb-4">
          <label class="block text-xs font-semibold text-gray-600 mb-2 uppercase tracking-wide">Kategorien zuweisen</label>
          <div class="flex flex-wrap gap-3">
            <label v-for="cat in CATEGORIES" :key="'c-' + cat.value" class="flex items-center gap-2 cursor-pointer">
...
async function sendConsentTest() {
  ...
  try {
    const res = await $fetch('/api/marketing/send-consent-test', {
      method: 'POST',
      body: { tenantId: tenantId.value, email: testEmail.value },
    }) as any
...
const userImportResult = ref<{ imported: number; skipped: number; total: number; error?: string } | null>(null)
...
async function importFromUsers() {
  ...
  try {
    const res = await $fetch('/api/marketing/import-from-users', {
      method: 'POST',
      body: { tenantId: tenantId.value, categories: userImportSelectedCategories.value },
    }) as any
...
async function importFromCustomers() {
  ...
  try {
    const res = await $fetch('/api/marketing/import-from-customers', {
      method: 'POST',
      body: { tenantId: tenantId.value, categories: customerImportSelectedCategories.value },
    }) as any
