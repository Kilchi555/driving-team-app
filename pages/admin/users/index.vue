        ...
        <form @submit.prevent="sendStaffInvitation" class="px-6 py-4 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
              :class="{
                '!border-red-300': inviteEmailCheck === 'taken',
                '!border-green-300': inviteEmailCheck === 'available',
              }"
              placeholder="z.B. vorname@gmail.com"
              @input="onInviteEmailInput(inviteForm.email)"
              @blur="checkInviteEmail(inviteForm.email)"
            />
            <p class="text-xs text-gray-500 mt-1">Nicht die Admin-E-Mail — sonst kann der Account nicht erstellt werden.</p>
            <p v-if="inviteEmailCheck === 'checking'" class="text-xs text-gray-400 mt-1">Wird geprüft…</p>
            <p v-else-if="inviteEmailCheck === 'available'" class="text-xs text-green-600 mt-1">E-Mail ist verfügbar</p>
            <p v-else-if="inviteEmailCheck === 'taken'" class="text-xs text-red-600 mt-1">{{ inviteEmailTakenMsg }}</p>
          </div>
...
const inviteEmailCheck = ref<'idle' | 'checking' | 'available' | 'taken' | 'error'>('idle')
const inviteEmailTakenMsg = ref('Diese E-Mail ist bereits registriert.')
let inviteEmailDebounce: ReturnType<typeof setTimeout> | null = null

const checkInviteEmail = (val: string) => {
  if (inviteEmailDebounce) clearTimeout(inviteEmailDebounce)
  const email = val.trim()
  if (!email.includes('@') || email.length < 5) {
    inviteEmailCheck.value = 'idle'
    return
  }
...
const onInviteEmailInput = (val: string) => {
  inviteEmailCheck.value = 'idle'
  checkInviteEmail(val)
}
...
const resendStaffInvitation = async (user: User) => {
  ...
  try {
    ...
    if (isPlaceholderInviteEmail(user.email)) {
      ...
      // Reuse the invite email already on the invitation
...
const sendStaffInvitation = async () => {
  ...
  if (!inviteForm.value.email?.trim()) {
    inviteStaffError.value = 'E-Mail für den Staff-Login ist erforderlich'
    return
  }
  if (inviteEmailCheck.value === 'taken') {
    inviteStaffError.value = inviteEmailTakenMsg.value
    return
  }
