      ...
      <div v-if="validationResult && importTarget === 'users' && canImport" class="bg-white rounded-lg shadow-sm border">
        <div class="p-6">
          ...
          <div v-if="dryRunResult && !dryRunResult.error" class="space-y-3">
            ...
            <!-- Erfolgreich korrigierte Zeilen -->
            <div v-if="correctedAndValid.length" class="overflow-hidden rounded-xl border border-green-200">
              <div class="bg-green-50 px-4 py-2.5 border-b border-green-200 flex items-center gap-2">
                ...
                <p class="text-sm font-medium text-green-800">
                  {{ correctedAndValid.length }} Zeile{{ correctedAndValid.length !== 1 ? 'n' : '' }} erfolgreich korrigiert — werden importiert
                </p>
              </div>
              <div class="divide-y divide-green-100 bg-white">
                <div v-for="item in correctedAndValid" :key="item.row" class="px-4 py-3 flex items-start gap-3">
                  <span class="text-xs font-mono text-gray-400 flex-shrink-0 mt-0.5">Zeile {{ item.row }}</span>
                  <div class="flex-1 min-w-0">
                    <div class="flex flex-wrap gap-x-4 gap-y-1">
                      <template v-for="(val, key) in item.correction" :key="key">
                        <div v-if="val" class="flex items-baseline gap-1">
                          <span class="text-xs text-gray-400 font-medium">{{ key }}:</span>
                          <span class="text-xs text-green-700 font-medium max-w-[200px] truncate" :title="val">{{ val }}</span>
                        </div>
                      </template>
                    </div>
                  </div>
                  <span class="flex items-center gap-1 text-xs font-medium text-green-700 bg-green-100 px-2 py-0.5 rounded-full flex-shrink-0">
                    <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                    Korrigiert
                  </span>
                </div>
                  ...
                  <div class="flex items-start justify-between gap-3 mb-2">
                    <div class="flex items-start gap-3 min-w-0 flex-wrap">
                      ...
                      <div class="flex items-center gap-2 min-w-0 flex-wrap">
                        ...
                        <!-- Badge: Korrektur fehlgeschlagen -->
                        <span v-if="correctedRows.has(inv.row - 2)"
                          class="flex items-center gap-1 text-xs font-medium text-orange-700 bg-orange-100 px-2 py-0.5 rounded-full">
                          <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                          Korrektur unvollständig
                        </span>
                      </div>
                    </div>
                    <!-- Aktions-Buttons -->
...
// Rows that were manually corrected AND now pass dry-run validation (no longer in invalids)
const correctedAndValid = computed(() => {
  if (!dryRunResult.value || correctedRows.size === 0) return []
  const stillInvalidRowNrs = new Set((dryRunResult.value.invalids || []).map((i: any) => i.row))
  return [...correctedRows.entries()]
    .filter(([rowArrayIdx]) => !stillInvalidRowNrs.has(rowArrayIdx + 2))
    .map(([rowArrayIdx, correction]) => ({
      row: rowArrayIdx + 2,
      correction,
    }))
})
