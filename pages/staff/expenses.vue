    ...
    <div class="max-w-lg mx-auto px-4 py-4 space-y-4" style="padding-left: max(1rem, env(safe-area-inset-left)); padding-right: max(1rem, env(safe-area-inset-right))">
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
        ...
        <label class="block text-xs font-medium text-gray-500 mb-1.5">
          Beleg / Quittung <span class="text-red-500">*</span>
        </label>
        ...
        <div v-if="form.receipt_url" class="flex items-center gap-3 p-2.5 bg-emerald-50 border border-emerald-200 rounded-xl">
          ...
          <button @click="resetReceipt" class="text-gray-400 hover:text-red-500 transition-colors p-1">
        ...
        <button v-if="!form.receipt_url" type="button" @click="fileInput?.click()"
          :disabled="uploading"
          class="w-full flex items-center gap-3 p-4 border-2 border-dashed rounded-xl transition-colors"
          :class="uploading ? 'border-emerald-300 bg-emerald-50' : 'border-gray-200 hover:border-emerald-400 active:bg-gray-50'">
          <svg v-if="!uploading" class="w-9 h-9 text-gray-300 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z"/>
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z"/>
          </svg>
          <svg v-else class="w-8 h-8 text-emerald-500 animate-spin flex-shrink-0" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/>
          </svg>
          <div class="text-left">
            <p class="text-sm font-semibold text-gray-700">{{ uploading ? 'Wird hochgeladen…' : 'Foto oder PDF hochladen' }}</p>
            <p class="text-xs text-gray-400 mt-0.5">JPG, PNG oder PDF</p>
          </div>
        </button>

        <!-- OCR spinner inside upload card -->
        <div v-if="parsing" class="flex items-center gap-2 mt-3">
      ...
      <!-- Step 2: Form fields — appear after upload + OCR done -->
      <Transition
        enter-active-class="transition-all duration-300 ease-out"
        enter-from-class="opacity-0 translate-y-3"
        enter-to-class="opacity-100 translate-y-0">
        <div v-if="showFields" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 space-y-3">

          <!-- OCR hint -->
          <div v-if="parseHint" class="flex items-center gap-2">
...
// Show the form fields only after a receipt has been uploaded and OCR is done
const showFields = computed(() => !!form.value.receipt_url && !uploading.value && !parsing.value)
