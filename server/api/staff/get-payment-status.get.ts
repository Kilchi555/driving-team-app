...
/**
 * ✅ GET /api/staff/get-payment-status
 * 
 * Secure API to check payment status (lightweight query)
 * 
 * Query Params:
 *   - id (required): Payment ID
 * 
 * Returns:
 *   - payment_status: string (pending, authorized, completed, failed, cancelled)
 *   - id: Payment ID
 * 
 * Security Layers:
...
export default defineEventHandler(async (event) => {
  try {
    ...
    // ✅ LAYER 4: DATABASE QUERY - lightweight (only status)
    ...
    if (error) {
      ...
      throw createError({
        statusCode: 500,
        statusMessage: 'Failed to fetch payment status'
      })
    }
    ...
    logger.error('❌ Staff get-payment-status API error:', error)
