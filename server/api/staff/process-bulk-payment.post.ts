...
export default defineEventHandler(async (event) => {
  try {
    ...
    // ✅ ROLE CHECK — only staff/admin may settle payments (cash/credit) in bulk
    ...
    if (method === 'credit') {
      ...
      let applied
      try {
        applied = await applyStudentCreditToPayments({
        ...
        throw createError({
          statusCode: 400,
          statusMessage: creditErr?.message || 'Guthaben konnte nicht verrechnet werden',
        })
      }

      if (applied.credit_used_rappen <= 0 && applied.allocations.length === 0) {
        throw createError({ statusCode: 400, statusMessage: 'Keine offenen Zahlungen zum Verrechnen' })
      }

      const results = (allPayments as any[]).map((p: any) => {
        const alloc = applied.allocations.find(a => a.payment_id === p.id)
        if (!alloc) return { payment_id: p.id, success: true, skipped: true }
        return {
          payment_id: p.id,
          success: true,
          status: alloc.fully_covered ? 'completed' : 'partial',
          credit_applied_rappen: alloc.apply_rappen,
        }
      })
      ...
      logger.debug('✅ Bulk credit payment completed:', {
        ...
        tenantId,
        totalCreditApplied: applied.credit_used_rappen,
        successCount: applied.allocations.length,
      })
      ...
      return {
        ...
        credit_remaining_rappen: applied.credit_remaining_rappen,
        summary: {
          total: payment_ids.length,
          successful: applied.allocations.length,
          failed: 0,
