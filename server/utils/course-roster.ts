...
export type CourseRosterQuery = {
  courseId?: string | null
  appointmentId?: string | null
}

/**
 * Loads course + sessions + enrollments for staff/admin roster views and PDFs.
 * Same access rules as GET /api/courses/roster.
 */
export async function loadCourseRoster(
  supabase: any,
  profile: CourseRosterProfile,
  query: CourseRosterQuery,
) {
  let courseId: string | null = typeof query.courseId === 'string' ? query.courseId : null
  let appointmentStart: string | null = null
  let appointmentEnd: string | null = null

  if (typeof query.appointmentId === 'string' && query.appointmentId) {
    const { data: apt, error: aptError } = await supabase
      .from('appointments')
      .select('id, notes, staff_id, tenant_id, event_type_code, start_time, end_time')
      .eq('id', query.appointmentId)
