...
<template>
  <div class="min-h-screen" :style="{ background: `linear-gradient(160deg, ${primaryColor}12 0%, #f9fafb 25%, #f3f4f6 100%)` }">
        ...
        >
          <div class="p-4 flex items-center">
            <div class="flex items-center">
              ...
              <h3 class="text-lg font-semibold text-gray-900">Lernen</h3>
            </div>
          </div>
        </div>
    ...
    <div v-if="showAffiliateModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
        <div class="p-6">
          ...
          <!-- Info Link -->
          <button
            @click="showAffiliateInfoModal = true"
            class="w-full mb-5 flex items-center gap-2 text-sm text-green-700 bg-green-50 hover:bg-green-100 rounded-lg px-3 py-2 transition text-left"
          >
            ...
            <span>Wie funktioniert das Empfehlungs- programm? <span class="underline">Mehr erfahren →</span></span>
          </button>
    ...
    <Teleport to="body">
      <div v-if="showAffiliateDetailModal" class="fixed inset-0 z-[200] bg-black bg-opacity-50 flex items-center justify-center p-4" @click.self="showAffiliateDetailModal = false">
        <div class="bg-white rounded-lg shadow-xl max-w-sm w-full max-h-[80vh] flex flex-col">
          <div class="border-b px-5 py-4 flex justify-between items-center shrink-0">
            <h3 class="font-semibold text-gray-900">{{ affiliateDetailTitle }}</h3>
            <button @click="showAffiliateDetailModal = false" class="text-gray-500 hover:text-gray-700 text-2xl leading-none font-bold">×</button>
          </div>
          <div class="overflow-y-auto p-4">
            <div v-if="filteredAffiliateDetail.length === 0" class="text-sm text-gray-400 text-center py-6">
              Keine Einträge
            </div>
            <div class="space-y-2">
              <!-- Earnings / credited: individual transactions -->
              <template v-if="affiliateDetailFilter === 'earnings' || affiliateDetailFilter === 'credited'">
                <div
                  v-for="tx in filteredAffiliateDetail"
                  :key="tx.id"
            ...
            <div v-if="(affiliateDetailFilter === 'earnings' || affiliateDetailFilter === 'credited') && filteredAffiliateDetail.length > 0" class="mt-3 pt-3 border-t flex justify-between items-center">
              <span class="text-sm font-semibold text-gray-600">Total verdient</span>
              <span class="text-sm font-bold text-green-700">
                CHF {{ (filteredAffiliateDetail.reduce((sum: number, t: any) => sum + (t.amount_rappen ?? 0), 0) / 100).toFixed(2) }}
              </span>
            </div>
          </div>
        </div>
    ...
    <Teleport to="body">
      <div v-if="showAffiliateInfoModal" class="fixed inset-0 z-[200] bg-black bg-opacity-50 flex items-center justify-center p-4" @click.self="showAffiliateInfoModal = false">
        <div class="bg-white rounded-xl shadow-xl max-w-sm w-full max-h-[85vh] flex flex-col">
          ...
          <div class="overflow-y-auto p-5 space-y-5">
            ...
            <div>
              ...
              <div v-if="affiliateLoading" class="text-xs text-gray-400 text-center py-3">Wird geladen…</div>
              <div v-else-if="affiliateRewards?.length" class="space-y-1.5">
                <div
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <div class="flex justify-between items-center gap-3">
        <button
          ...
          class="flex items-center space-x-2 px-4 py-2 font-medium border rounded-lg transition-colors"
          :style="{
            color: primaryColor,
            borderColor: primaryColor,
            background: `${primaryColor}10`
          }"
          @mouseover="($event.currentTarget as HTMLElement)?.style.setProperty('background', `${primaryColor}20`)"
          @mouseleave="($event.currentTarget as HTMLElement)?.style.setProperty('background', `${primaryColor}10`)"
        >
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
          </svg>
          <span class="font-medium">Reglemente</span>
        </button>
        <button
          @click="handleLogout"
          :disabled="isLoggingOut"
          class="flex items-center space-x-2 px-4 py-2 bg-red-50/60 text-red-600 font-medium border border-red-400 rounded-lg hover:bg-red-100/80 disabled:opacity-50 transition-colors"
        >
          <svg 
            class="w-5 h-5" 
...
const affiliateRewards = computed(() => {
  if (!affiliateStats.value) return null
  return (affiliateStats.value as any).category_rewards ?? []
})
...
const filteredAffiliateDetail = computed(() => {
  if (affiliateDetailFilter.value === 'earnings' || affiliateDetailFilter.value === 'credited') {
