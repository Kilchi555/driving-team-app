<template>
  ...
  <div v-if="isOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999] p-2 sm:p-4" style="padding-top: max(24px, env(safe-area-inset-top, 24px)); padding-bottom: max(24px, env(safe-area-inset-bottom, 24px));">
    <div class="bg-white rounded-2xl shadow-2xl max-w-4xl w-full flex flex-col evaluation-modal-container" style="height: 100%; max-height: 100%;">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2">
            ...
            <button 
              ...
              class="px-3 py-1.5 text-sm font-medium text-red-600 bg-red-50 hover:bg-red-100 rounded-xl transition-colors"
      ...
      <!-- Sortierungs-Regler -->
      <div v-if="currentTermCriteria.length > 1" class="flex items-center justify-between px-4 py-2 bg-gray-50 border-b flex-shrink-0">
        <div class="flex items-center space-x-2">
          <span class="text-gray-400 text-xs font-medium">Sortierung:</span>
          <button
            @click="sortByNewest = true"
            :class="[
              'px-3 py-1 text-xs rounded-lg transition-colors',
              sortByNewest ? 'text-white' : 'bg-white text-gray-600 hover:bg-gray-100 border border-gray-200'
            ]"
            :style="sortByNewest ? primaryBg : {}"
          >
            Nach Datum
          </button>
          <button
          ...
          <div class="relative">
            <div class="relative">
              <input
                ...
                class="search-input w-full pl-10 pr-2 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:border-transparent"
      ...
      <div class="bg-white px-4 py-3 border-t border-gray-100 flex-shrink-0 evaluation-modal-footer">
        <div class="flex gap-3">
          ...
          <button
            ...
            :class="[
              ...
              !(isValid && !isSaving) && 'bg-gray-200 text-gray-400 cursor-not-allowed'
            ]"
            :style="isValid && !isSaving ? primaryBg : {}"
          >
