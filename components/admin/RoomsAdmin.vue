<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between flex-wrap gap-3">
      <div>
        <h2 class="text-xl font-bold text-gray-900">Räume</h2>
        <p class="text-sm text-gray-500 mt-0.5">Raumverwaltung und Buchungskalender</p>
      </div>
        ...
        >
          ...
          <div class="flex flex-wrap gap-1.5">
            ...
            <span v-else-if="+room.hourly_rate_rappen > 0" class="text-xs font-semibold px-2 py-0.5 rounded-full"
              :style="{ color: primaryColor, background: primaryColor + '18' }">
              CHF {{ (room.hourly_rate_rappen / 100).toFixed(2) }}/h
            </span>
