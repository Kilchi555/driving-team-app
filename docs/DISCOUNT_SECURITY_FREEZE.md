...
**When to use:** Clients bump `usage_count` / `current_redemptions`; `/api/discounts/apply/:id` still mutates counters; customers apply pending appointment discounts; debugging abusive discount usage after #248.

Verified against source (Sep 2026). Merge `467e1c1a` (#248). Migration file: `migrations/20260920_pra_discount_security_freeze.sql` (create-only; confirm production apply separately).

Related but separate:

- Fixed-value **units** (CHF vs rappen) — #246 / `server/utils/discount-amount.ts` (not this freeze).
...
This freeze does **not** add a `discount_usages` ledger, reservation state, or new staff apply privilege.

---

## Intent

Three critical gaps let untrusted clients influence redemption math:
...
- `SECURITY DEFINER` triggers allow mutation only when JWT role is `service_role`, or SQL has **no** JWT context (migrations / console).
- Authenticated / anon / PUBLIC: INSERT forces counter to `0`; UPDATE that changes the counter raises `42501`.
...
Client-side inserts that name `usage_count: 0` (e.g. shop purchase → `utils/paymentService.ts` voucher row) are OK: INSERT trigger still forces `0`; clients must not **update** the counter.
...
4. Staff/admin → **403** with existing message (`Nur Kunden können Rabattcodes anwenden`) — no new staff privilege.

`pages/customer/payments.vue` no longer calls apply-discount; customers can still **see** discounts.
...
| Shop `applyDiscountCode` | Client validate + attach to cart; does not hit C2/C3 routes |
| Booking / public payment discount resolve | Server-side price math (see #246 units) |
| `server/api/wallee/webhook.post.ts` | Service-role increments `usage_count` / `current_redemptions` after paid fulfillment |
...
2. **Trusting body role on apply-discount** — C3 explicitly ignores body privilege flags.
...
5. **Expecting a usage ledger** — PR-A deliberately does not introduce `discount_usages` or reservations; counter integrity is freeze + trusted writers only.
...
7. **Production SQL** — shipping the app code without applying `20260920_pra_discount_security_freeze.sql` leaves DB-level C1 open even if manage stripping is deployed.
...
| C3 UI | `pages/customer/payments.vue` (pending apply removed) |
| Trusted increment | `server/api/wallee/webhook.post.ts` (service-role) |
