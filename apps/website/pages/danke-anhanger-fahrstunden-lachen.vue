<template>
  <div>
    <Head>
      <Title>Danke - Anhänger Fahrstunden Lachen | Driving Team</Title>
      <Meta name="robots" content="noindex, nofollow" />
          <Meta property="og:image" content="https://drivingteam.ch/images/og-image.webp" />
      <Meta property="og:image:width" content="1200" />
      <Meta property="og:image:height" content="630" />
    </Head>
    <section class="section-container py-20 text-center">
      <h1 class="heading-lg mb-8">Danke!</h1>
      <p class="text-lg text-gray-700 mb-8">Vielen Dank für deine Anmeldung zu Anhänger Fahrstunden in Lachen!</p>
      <p class="text-gray-600 mb-8">Wir werden dich in Kürze kontaktieren, um die Details zu klären.</p>
      <a href="/" class="btn-primary">Zur Startseite</a>
    </section>
  </div>
</template>
<script setup lang="ts">// Danke Anhänger Fahrstunden Lachen</script>
