<template>
  <div>
    <Head>
      <Title>LKW Fahrschule Kategorie C / C1 / CE & CZV Kurse | Driving Team</Title>
      <Meta name="description" content="LKW Fahrschule Kategorie C3 beim Driving Team. CZV Grundkurs und berufliche Weiterbildung für Berufschauffeure. Zürich und Lachen. Kontaktiere uns!" />
      <Meta property="og:title" content="LKW Fahrschule Kategorie C3 – CZV Grundkurs | Driving Team" />
      <Meta property="og:description" content="LKW Fahrschule Kategorie C3 inkl. CZV Grundkurs. Berufliche Weiterbildung in Zürich & Lachen." />
      <Meta property="og:url" content="https://drivingteam.ch/lastwagen-fahrschule-3/" />
      <Link rel="canonical" href="https://drivingteam.ch/lastwagen-fahrschule-3/" />
      
          <Meta property="og:image" content="https://drivingteam.ch/images/categories/lastwagen-fahrschule-hero.webp" />
      <Meta property="og:image:width" content="1200" />
      <Meta property="og:image:height" content="630" />
          <Meta name="robots" content="noindex, nofollow" />
    </Head>

    <!-- Hero -->
    <section class="bg-gradient-to-br from-primary-600 to-primary-800 text-white py-16 md:py-24">
      <div class="section-container">
        <div class="max-w-3xl">
          <h1 class="heading-lg text-white mb-6">LKW Fahrschule & CZV Grundkurs</h1>
          <p class="text-xl text-white mb-8">Berufliche Ausbildung und Weiterbildung für Berufschauffeure in Zürich und Lachen/SZ. Kat. C, CZV Grundkurs und mehr.</p>
          <a href="https://app.simy.ch/booking/availability/driving-team?category=C" target="_blank" rel="noopener noreferrer" class="btn-primary bg-white text-primary-600 hover:bg-primary-50 text-lg">
            ✨ Jetzt anmelden
          </a>
        </div>
      </div>
    </section>

    <!-- Links zu Seiten -->
    <section class="section-container">
      <h2 class="heading-md mb-10 text-center">Unser Angebot für LKW Fahrer</h2>
      <div class="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
        <a href="/lastwagen-fahrschule/" class="bg-white border-2 border-gray-200 rounded-lg p-8 hover:shadow-lg hover:border-primary-400 transition">
          <p class="text-4xl mb-4">🚛</p>
          <h3 class="heading-sm mb-2">LKW Fahrschule Kat. C / C1 / CE</h3>
          <p class="text-gray-600">Vollständige LKW Fahrausbildung in 5 Schritten. Versicherungspauschale ab CHF 160.-</p>
          <p class="text-primary-600 font-semibold mt-2">Mehr erfahren →</p>
        </a>
        <a href="/czv-grundkurs/" class="bg-white border-2 border-gray-200 rounded-lg p-8 hover:shadow-lg hover:border-primary-400 transition">
          <p class="text-4xl mb-4">🎓</p>
          <h3 class="heading-sm mb-2">CZV Grundkurs</h3>
          <p class="text-gray-600">Obligatorischer Grundkurs für Berufschauffeure.</p>
          <p class="text-primary-600 font-semibold mt-2">Mehr erfahren →</p>
        </a>
        <a href="/czv-weiterbildung/" class="bg-white border-2 border-gray-200 rounded-lg p-8 hover:shadow-lg hover:border-primary-400 transition">
          <p class="text-4xl mb-4">🔄</p>
          <h3 class="heading-sm mb-2">CZV Weiterbildung</h3>
          <p class="text-gray-600">5 Kurse in 5 Jahren. Prävention, Traffic Health, ARV1, Radwechsel. CHF 190.- pro Tag.</p>
          <p class="text-primary-600 font-semibold mt-2">Mehr erfahren →</p>
        </a>
        <a href="/lastwagen-theorie/" class="bg-white border-2 border-gray-200 rounded-lg p-8 hover:shadow-lg hover:border-primary-400 transition">
          <p class="text-4xl mb-4">📚</p>
          <h3 class="heading-sm mb-2">LKW Theorie</h3>
          <p class="text-gray-600">Theorievorbereitung für Kat. C. Gruppen- und Einzelkurse. Ab CHF 800.- bei Gruppenkurse.</p>
          <p class="text-primary-600 font-semibold mt-2">Mehr erfahren →</p>
        </a>
      </div>
    </section>

    <!-- CTA -->
    <section class="bg-primary-600 text-white py-16">
      <div class="section-container text-center">
        <h2 class="heading-md mb-6 text-white">Lastwagen Ausbildung starten!</h2>
        <p class="text-xl text-white mb-8">Professionelle LKW Ausbildung in Zürich und Lachen/SZ</p>
        <div class="flex flex-col md:flex-row gap-4 justify-center">
          <a href="https://app.simy.ch/booking/availability/driving-team?category=C" target="_blank" rel="noopener noreferrer" class="btn-primary bg-white text-primary-600 hover:bg-primary-50 text-lg">📅 Jetzt anmelden</a>
          <a href="tel:+41444310033" class="btn-primary bg-transparent border-2 border-white text-white hover:bg-white hover:text-primary-600 text-lg">📞 +41 44 431 00 33</a>
        </div>
      </div>
    </section>
  </div>
</template>

<script setup lang="ts">
const jsonLdScripts = [
  { type: 'application/ld+json', innerHTML: JSON.stringify({ "@context": "https://schema.org", "@type": "Course", "name": "LKW Fahrschule Kategorie C3", "description": "LKW Fahrausbildung Kategorie C3 mit CZV Grundkurs.", "url": "https://drivingteam.ch/lastwagen-fahrschule-3/", "provider": { "@type": "Organization", "name": "Driving Team Fahrschule", "url": "https://drivingteam.ch" } }) },
  { type: 'application/ld+json', innerHTML: JSON.stringify({ "@context": "https://schema.org", "@type": "BreadcrumbList", "itemListElement": [{ "@type": "ListItem", "position": 1, "name": "Home", "item": "https://drivingteam.ch/" }, { "@type": "ListItem", "position": 2, "name": "Lastwagen Fahrschule", "item": "https://drivingteam.ch/lastwagen-fahrschule/" }, { "@type": "ListItem", "position": 3, "name": "LKW Kategorie C3", "item": "https://drivingteam.ch/lastwagen-fahrschule-3/" }] }) },
]
useHead({ script: jsonLdScripts })

</script>
