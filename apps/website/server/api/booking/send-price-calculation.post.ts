  ...
  try {
    ...
    try {
      ...
      await resend.emails.send({
        from: fromEmail,
        to: body.email.trim(),
        subject: 'Deine Kostenschätzung – Driving Team Fahrschule',
...
function buildCustomerEmail(body: PriceCalculationPayload): string {
  ...
  const rows = body.calculationDetails
    ...
    .map(line => {
      if (line.startsWith('GESCHÄTZTE GESAMTKOSTEN')) {
        return `<tr style="background:#1C64F2;"><td style="padding:12px 16px;font-size:14px;font-weight:700;color:white;">${line.replace('CHF', '').replace('GESCHÄTZTE GESAMTKOSTEN:', 'Geschätzte Gesamtkosten').trim()}</td><td style="padding:12px 16px;font-size:16px;font-weight:700;color:white;text-align:right;white-space:nowrap;">CHF ${body.totalCost}.–</td></tr>`
      }
      ...
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f3f4f6;">
        <tr><td align="center" style="padding:20px 10px;">
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;background:white;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
            ...
            <!-- Header -->
            <tr>
              <td style="background:linear-gradient(135deg,${PRIMARY_COLOR} 0%,${PRIMARY_COLOR}cc 100%);color:white;padding:28px 20px;text-align:center;">
                <h1 style="margin:0;font-size:22px;font-weight:700;">💰 Deine Kostenschätzung</h1>
                <p style="margin:6px 0 0;font-size:14px;opacity:0.9;">${TENANT_NAME}</p>
              </td>
            </tr>
              ...
              <td style="padding:28px 20px;">
                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f9fafb;border-radius:8px;margin-bottom:24px;">
                  <tr><td style="padding:20px;text-align:center;">
                    ...
                    <a href="https://simy.ch/booking/availability/driving-team" style="display:inline-block;padding:12px 28px;background:${PRIMARY_COLOR};color:white;text-decoration:none;border-radius:8px;font-size:14px;font-weight:600;">Jetzt Termin buchen →</a>
                  </td></tr>
                </table>
...
function buildTeamEmail(body: PriceCalculationPayload): string {
    ...
    <body style="margin:0;padding:0;background-color:#f3f4f6;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;">
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f3f4f6;">
        <tr><td align="center" style="padding:20px 10px;">
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;background:white;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
            <tr>
              <td style="background:linear-gradient(135deg,${PRIMARY_COLOR} 0%,${PRIMARY_COLOR}cc 100%);color:white;padding:25px 20px;text-align:center;">
              ...
              <td style="padding:24px 20px;">
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f9fafb;border-radius:8px;margin-bottom:20px;">
                  <tr><td style="padding:20px;">
                    <h3 style="margin:0 0 14px;color:${PRIMARY_COLOR};font-size:14px;font-weight:600;text-transform:uppercase;letter-spacing:.5px;">Lead-Daten</h3>
                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
                      ...
                      <tr><td style="padding:4px 0;font-size:13px;color:#6b7280;">E-Mail</td><td style="padding:4px 0;font-size:13px;"><a href="mailto:${body.email}" style="color:${PRIMARY_COLOR};">${body.email}</a></td></tr>
                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-bottom:16px;">
                  <tr>
                    <td style="padding:4px;">
                      <a href="mailto:${body.email}?subject=Deine Kostenschätzung – Driving Team Fahrschule" style="display:inline-block;padding:10px 20px;background:${PRIMARY_COLOR};color:white;text-decoration:none;border-radius:6px;font-size:13px;font-weight:600;">✉️ Follow-up senden</a>
                    </td>
                  </tr>
                </table>
